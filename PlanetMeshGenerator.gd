...
func _ready():
	...
	var shader_file = load("res://planet_compute_heights.glsl")
...
func update_heights(resolution):
	...
	var parameters_array := PackedFloat32Array(
		[
			...
			get_parent().o_floor_depth,
			get_parent().o_floor_smoothing, 
			get_parent().o_depth_multiplier,
			get_parent().mountain_blend,
			
			get_parent().c_num_layers,
			get_parent().c_scale,
			get_parent().c_persistence,
			get_parent().c_lacunarity,
			get_parent().c_multiplier,
			
			get_parent().m_offset.x,
			get_parent().m_offset.y,
			get_parent().m_offset.z,
			get_parent().m_num_layers,
			get_parent().m_persistence,
			get_parent().m_lacunarity,
			get_parent().m_scale,
			get_parent().m_multiplier,
			get_parent().m_gain,
			get_parent().m_power,
			get_parent().m_vertical_shift,
			
			get_parent().ma_num_layers,
			get_parent().ma_scale,
			get_parent().ma_persistence,
			get_parent().ma_lacunarity,
			get_parent().ma_multiplier,
			get_parent().ma_vertical_shift,
		]
	)
	...
	var uniform_set := rd.uniform_set_create(
		[vertex_uniform, parameter_uniform, heights_uniform],
		shader, 0
	)
